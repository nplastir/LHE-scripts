...
def create_plots(input_root_file, output_directory):
    ...
    dd_system_histograms = create_histograms("dd_system", quarks_system_info)
    uu_system_histograms = create_histograms("uu_system", quarks_system_info)
    ss_system_histograms = create_histograms("ss_system", quarks_system_info)
    cc_system_histograms = create_histograms("cc_system", quarks_system_info)
    ud_system_histograms = create_histograms("ud_system", quarks_system_info)
    sc_system_histograms = create_histograms("sc_system", quarks_system_info)
    cd_system_histograms = create_histograms("cd_system", quarks_system_info)
    su_system_histograms = create_histograms("su_system", quarks_system_info)
    uc_system_histograms = create_histograms("uc_system", quarks_system_info)
    ds_system_histograms = create_histograms("ds_system", quarks_system_info)
    ...
    dubar_pairs_system_histograms = create_histograms("dubar_pairs_system", quark_pairs_system_info)
    dbaru_pairs_system_histograms = create_histograms("dbaru_pairs_system", quark_pairs_system_info)
    scbar_pairs_system_histograms = create_histograms("scbar_pairs_system", quark_pairs_system_info)
    sbarc_pairs_system_histograms = create_histograms("sbarc_pairs_system", quark_pairs_system_info)
    subar_pairs_system_histograms = create_histograms("subar_pairs_system", quark_pairs_system_info)
    sbaru_pairs_system_histograms = create_histograms("sbaru_pairs_system", quark_pairs_system_info)
    dcbar_pairs_system_histograms = create_histograms("dcbar_pairs_system", quark_pairs_system_info)
    dbarc_pairs_system_histograms = create_histograms("dbarc_pairs_system", quark_pairs_system_info)
    pairs_from_Wm_histograms = create_histograms("pairs_from_Wm", quark_pairs_system_info)
    pairs_from_Wp_histograms = create_histograms("pairs_from_Wp", quark_pairs_system_info)
    ...
    histogram_dict = {
        ...
        "dd_system": dd_system_histograms,
        "uu_system": uu_system_histograms,
        "ss_system": ss_system_histograms,
        "cc_system": cc_system_histograms,
        "ds_system": ds_system_histograms,
        "uc_system": uc_system_histograms,
        "ud_system": ud_system_histograms,
        "sc_system": sc_system_histograms,
        "cd_system": cd_system_histograms,
        "su_system": su_system_histograms,  
        ...
        "dubar_pairs_system": dubar_pairs_system_histograms,
        "dbaru_pairs_system": dbaru_pairs_system_histograms,
        "scbar_pairs_system": scbar_pairs_system_histograms,
        "sbarc_pairs_system": sbarc_pairs_system_histograms,
        "subar_pairs_system": subar_pairs_system_histograms,
        "sbaru_pairs_system": sbaru_pairs_system_histograms,
        "dcbar_pairs_system": dcbar_pairs_system_histograms,
        "dbarc_pairs_system": dbarc_pairs_system_histograms,
        "pairs_from_Wm": pairs_from_Wm_histograms,
        "pairs_from_Wp": pairs_from_Wp_histograms,
    ...
    for event in tree:
        ...
        for i in range(len(event.px)):
            ...
            for j in range(len(event.px)):
                ...
                if event.mother1[i] > 2 and event.mother1[j] > 2:
                    if event.pid[i] == 1 or event.pid[i] == -1:
                        if event.pid[j] == 1 or event.pid[j] == -1:
                            if i < j:
                                fill_system_histograms(dd_system_histograms, event, i,j)
                    
                    if event.pid[i] == 2 or event.pid[i] == -2:
                        if event.pid[j] == 2 or event.pid[j] == -2:
                            if i < j:
                                fill_system_histograms(uu_system_histograms, event, i,j)

                    if event.pid[i] == 3 or event.pid[i] == -3:
                        if event.pid[j] == 3 or event.pid[j] == -3:
                            if i < j:
                                fill_system_histograms(ss_system_histograms, event, i,j)

                    if event.pid[i] == 4 or event.pid[i] == -4:
                        if event.pid[j] == 4 or event.pid[j] == -4:
                            if i < j:
                                fill_system_histograms(cc_system_histograms, event, i,j)

                    if event.pid[i] == 1 or event.pid[i] == -1:
                        if event.pid[j] == 3 or event.pid[j] == -3:
                            if temp == 0:
                                fill_system_histograms(ds_system_histograms, event,i,j)
                                temp += 1
                    
                    if event.pid[i] == 2 or event.pid[i] == -2:
                        if event.pid[j] == 4 or event.pid[j] == -4:
                            if temp == 0:
                                fill_system_histograms(uc_system_histograms, event,i,j)
                                temp += 1

                    if (event.pid[i] == 1 and event.pid[j] == 2) or (event.pid[i] == -1 and event.pid[j] == -2):
                        fill_system_histograms(ud_system_histograms, event, i,j)
                    
                    if (event.pid[i] == 3 and event.pid[j] == 4) or (event.pid[i] == -3 and event.pid[j] == -4):
                        fill_system_histograms(sc_system_histograms, event, i,j)

                    if (event.pid[i] == 3 and event.pid[j] == 2) or (event.pid[i] == -3 and event.pid[j] == -2):
                        fill_system_histograms(su_system_histograms, event, i,j)
                    
                    if (event.pid[i] == 1 and event.pid[j] == 4) or (event.pid[i] == -1 and event.pid[j] == -4):
                        fill_system_histograms(cd_system_histograms, event, i,j)

                    if (event.pid[i] == 1 and event.pid[j] == -2):
                        fill_system_histograms(dubar_pairs_system_histograms, event, i,j)
                    if (event.pid[i] == -1 and event.pid[j] == 2):
                        fill_system_histograms(dbaru_pairs_system_histograms, event, i,j)
                    
                    if (event.pid[i] == 3 and event.pid[j] == -4):
                        fill_system_histograms(scbar_pairs_system_histograms, event, i,j)
                    if (event.pid[i] == -3 and event.pid[j] == 4):
                        fill_system_histograms(sbarc_pairs_system_histograms, event, i,j)
                    
                    if (event.pid[i] == 3 and event.pid[j] == -2):
                        fill_system_histograms(subar_pairs_system_histograms, event, i,j)
                    if (event.pid[i] == -3 and event.pid[j] == 2):
                        fill_system_histograms(sbaru_pairs_system_histograms, event, i,j)
                    
                    if (event.pid[i] == 1 and event.pid[j] == -4):
                        fill_system_histograms(dcbar_pairs_system_histograms, event, i,j)
                    if (event.pid[i] == -1 and event.pid[j] == 4):
                        fill_system_histograms(dbarc_pairs_system_histograms, event, i,j)

                    if (event.pid[i] == 1 and event.pid[j] == -2) or (event.pid[i] == 3 and event.pid[j] == -4) or (event.pid[i] == 3 and event.pid[j] == -2) or (event.pid[i] == 1 and event.pid[j] == -4):
                        fill_system_histograms(pairs_from_Wm_histograms, event, i,j)

                    if (event.pid[i] == -1 and event.pid[j] == 2) or (event.pid[i] == -3 and event.pid[j] == 4) or (event.pid[i] == -3 and event.pid[j] == 2) or (event.pid[i] == -1 and event.pid[j] == 4):
                        fill_system_histograms(pairs_from_Wp_histograms, event, i,j)
