...
def create_plots(input_root_file, output_directory):
    ...
    #Load the root file and open the tree 
    ...
    #====================================================================================================================
    #Dictionaries with all the necessary info for the automatic creation of the histograms
    ...
    #====================================================================================================================
    ## Create the histograms for all the above
    ...
    ll_system_histograms = create_histograms("ll_system", ll_system_info)
    ee_system_histograms = create_histograms("ee_system", ll_system_info)
    ...
    tautau_system_histograms = create_histograms("tautau_system", ll_system_info)
    et_system_histograms = create_histograms("et_system", ll_system_info)
    mt_system_histograms = create_histograms("mt_system", ll_system_info)
    ...
    histogram_dict = {
        ...
        "ll_system": ll_system_histograms,
        "ee_system": ee_system_histograms,
        "mm_system": mm_system_histograms,
        "tautau_system": tautau_system_histograms,
        "em_system": em_system_histograms,
        "et_system": et_system_histograms,
        "mt_system": mt_system_histograms,
    ...
    for event in tree:
        ...
        for i in range(len(event.px)):
            ...
            for j in range(len(event.px)):
                ...
                if event.pid[i] == 11 or event.pid[i] == -11 or event.pid[i] == 13 or event.pid[i] == -13 or event.pid[i] == 15 or event.pid[i] == -15:
                    if event.pid[j] == 11 or event.pid[j] == -11 or event.pid[j] == 13 or event.pid[j] == -13 or event.pid[j] == 15 or event.pid[j] == -15:
                        if  i < j: # avoid double counting 
                            fill_system_histograms(ll_system_histograms, event, i, j)
                
                if event.pid[i] == 11 or event.pid[i] == -11:
                    if event.pid[j] == 11 or event.pid[j] == -11:
                        if i < j: # avoid double counting 
                            fill_system_histograms(ee_system_histograms, event, i, j)

                if event.pid[i] == 13 or event.pid[i] == -13:
                    if event.pid[j] == 13 or event.pid[j] == -13:
                        if i < j: # avoid double counting 
                            fill_system_histograms(mm_system_histograms, event, i, j)

                if event.pid[i] == 15 or event.pid[i] == -15:
                    if event.pid[j] == 15 or event.pid[j] == -15:
                        if i < j: # avoid double counting 
                            fill_system_histograms(tautau_system_histograms, event, i, j)
                
                if event.pid[i] == 11 or event.pid[i] == -11:
                    if event.pid[j] == 13 or event.pid[j] == -13:
                        if temp == 0: # avoid double counting 
                ...
                if event.pid[i] == 11 or event.pid[i] == -11:
                    if event.pid[j] == 15 or event.pid[j] == -15:
                        if temp == 0: # avoid double counting
                            fill_system_histograms(et_system_histograms, event, i, j)
                            temp += 1

                if event.pid[i] == 13 or event.pid[i] == -13:
                    if event.pid[j] == 15 or event.pid[j] == -15:
                        if temp == 0: # avoid double counting
                            fill_system_histograms(mt_system_histograms, event, i, j)
                            temp += 1
    ...
    for name, histograms in histogram_dict.items():
        draw(histograms, name, output_directory) # use the draw function if you want linear scale in the y axis
